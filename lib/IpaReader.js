...
IpaReader.prototype.parse = function (callback) {
    ...
    this.getEntries(whatYouNeed, function (error, buffers) {
        ...
        if (that.options.withIcon) {
            if (that.options.iconType !== 'node-base64' && (typeof Blob === 'undefined' || typeof document === 'undefined' || typeof URL === 'undefined')) {
                ...
                return that.getEntry(new RegExp(iconName.toLowerCase()), function (error, blob) {
                    ...
                    if (blob) {
                        if (that.options.iconType === 'node-base64' && typeof Buffer !== 'undefined' && Buffer.isBuffer(blob)) {
                            plistInfo.icon = blob.toString('base64');
